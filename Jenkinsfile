pipeline {
    ...
    stages {
        ...
        stage('Push to ECR') {
            steps{
                script {
                    docker.withRegistry( "https://$ecrAddress/$registry", "ecr:$awsRegion:$awsCredential" ) {
                        dockerImage.push()
                    }
                }
            }
            ...
            steps {
                withAWS(region:"$awsRegion",credentials:'aws') {
                    sh '''
                       ...
                       kubectl apply -f hello.yaml -n $namespace
                       kubectl rollout status deploy/hello --timeout=600s -n $namespace
                       '''                    
                }
            }
        }
